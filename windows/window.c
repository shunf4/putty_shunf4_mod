 ...
 *   ----------------+------------------+-------------------
 *   GLYPH_HALF      | NORMAL           | NORMAL + centre  (CENTRE strategy)
 *   |               |                  | WIDE             (WIDE strategy)
 *   GLYPH_FULL      | NARROW           | NORMAL
 *   GLYPH_XWIDE     | NARROW (no better)| NARROW (compress)
 *
 * The half-width + full-target strategy is selected by
 * overflow_fallback_half_glyph_strategy (default OVERFLOW_WIDE:
 * monochrome emoji glyphs in fallback fonts are mostly full-width
 * and benefit from being stretched to fill the two-cell span).
 * With OVERFLOW_CENTRE the glyph is rendered at natural NORMAL size
 * and shifted right to centre it instead.  An over-wide glyph can
 * only be compressed via NARROW — there is no tighter variant, so
 * GLYPH_XWIDE always maps to NARROW regardless of target.
 *
                                    ...
                                    int *x_offset_out)
{
    ...
    for (int i = 0; i < wgs->fallback_font_count; i++) {
        ...
        switch (gclass) {
          case GLYPH_HALF:
            if (target_full &&
                overflow_fallback_half_glyph_strategy == OVERFLOW_WIDE)
    ...
    int nfont, COLORREF fg, COLORREF bg)
{
    ...
    /*
     * Centring offset for a half-width main-font glyph (the fallback
     * path already computed x_off above).  Only active when
     * overflow_main_font_half_glyph_strategy == OVERFLOW_CENTRE; the
     * WIDE strategy stretches instead.  Full and xwide glyphs need no
     * shift either way.
     */
            ...
            if (wgs->fallback_font_count > 0) {
                ...
                for (int i = 0; i < len; ) {
                    ...
                    if (glyph_idx[i] == 0xFFFF || glyph_idx[i] == 0) {
                        /*
                         * If this run is an overflow cell, the normal
